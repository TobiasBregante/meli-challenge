...
const Sell = ({ products, website, equipamiento }) => {
    return (
        <Page categories={website?.categories}>
            <Container lg css={{ mb: "$10" }}>
                <CategoriesCarousel data={website?.categories}/>
                <UnorderedList title="Venta" data={products} />
...
export async function getServerSideProps(ctx) {
    ...
    return {
        props: {
            products: await Get("products/find/query?popular=false&premiunOnly=true&limit=200&isPublic=true&product_category=Venta").then(r => r.data).catch(() => []),
